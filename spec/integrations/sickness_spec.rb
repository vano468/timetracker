...
RSpec.describe Worktime, type: :view do
  ...
  describe 'sickness form page', type: :feature do
    let(:record) do
      {
        from: Time.now,
    ...
    it 'adds new sickness record' do
      visit '/records/new/sickness'
      fill_and_submit_form(form_id, record)
      check_page(page, record)
    end

    it 'can\'t add wrong sickness record' do 
      visit '/records/new/sickness'
  ...
  def fill_and_submit_form(id, record)
    within id do
      fill_in('sickness_date_from', with: record[:from].strftime(DATE_FORMAT))
      fill_in('sickness_date_to', with: record[:to].strftime(DATE_FORMAT))
      fill_in('sickness_emails', with: record[:emails])
      fill_in('sickness_comment', with: record[:comment])
