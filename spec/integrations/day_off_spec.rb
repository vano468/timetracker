...
RSpec.describe Worktime, type: :view do
  ...
  describe 'day off form page', type: :feature do
    let(:record) do
      {
        from: Time.now,
    ...
    it 'adds new day off record' do
      visit '/records/new/day_off'
      fill_and_submit_form(form_id, record)
      check_page(page, record)
    end

    it 'can\'t add wrong day off record' do
      visit '/records/new/day_off'
  ...
  def fill_and_submit_form(id, record)
      within id do
        fill_in('day_off_date_from', with: record[:from].strftime(DATE_FORMAT))
        fill_in('day_off_date_to', with: record[:to].strftime(DATE_FORMAT))
        fill_in('day_off_emails', with: record[:emails])
        fill_in('day_off_comment', with: record[:comment])
